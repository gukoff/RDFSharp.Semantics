...
namespace RDFSharp.Semantics.Reasoner.Test
{
    ...
    public class OWLReasonerRuleAbsBuiltInTest
    {
        ...
        public void ShouldThrowExceptionOnCreatingAbsBuiltInBecauseNullLeftArgument()
            => Assert.ThrowsException<OWLSemanticsException>(() => new OWLReasonerRuleAbsBuiltIn(null, new RDFVariable("?R")));

        [TestMethod]
        public void ShouldThrowExceptionOnCreatingAbsBuiltInBecauseNullRightArgument()
            => Assert.ThrowsException<OWLSemanticsException>(() => new OWLReasonerRuleAbsBuiltIn(new RDFVariable("?L"), null));
