...
namespace RDFSharp.Semantics
{
    ...
    /// <summary>
    /// OWLReasonerRuleDifferentFromAtom represents an atom inferring owl:differentFrom assertions between ontology facts 
    /// </summary>
    public class OWLReasonerRuleDifferentFromAtom : OWLReasonerRuleObjectPropertyAtom
    {
        ...
        public OWLReasonerRuleDifferentFromAtom(RDFVariable leftArgument, RDFVariable rightArgument)
            : base(RDFVocabulary.OWL.DIFFERENT_FROM.ToOWLOntologyObjectProperty(), leftArgument, rightArgument) { }

        /// <summary>
        /// Default-ctor to build an owl:differentFrom atom with the given arguments
        /// </summary>
        public OWLReasonerRuleDifferentFromAtom(RDFVariable leftArgument, OWLOntologyFact rightArgument)
            : base(RDFVocabulary.OWL.DIFFERENT_FROM.ToOWLOntologyObjectProperty(), leftArgument, rightArgument) { }
        #endregion
        ...
        internal override DataTable EvaluateOnAntecedent(OWLOntology ontology, OWLOntologyReasonerOptions options)
        {
            ...
            if (this.RightArgument is OWLOntologyFact rightArgumentFact)
            {
                OWLOntologyData differentFacts = OWLOntologyHelper.GetDifferentFactsFrom(ontology.Data, rightArgumentFact);
                foreach (OWLOntologyFact differentFact in differentFacts)
                {
            ...
            {
                foreach (OWLOntologyFact ontologyFact in ontology.Data)
                {
                    OWLOntologyData differentFacts = OWLOntologyHelper.GetDifferentFactsFrom(ontology.Data, ontologyFact);
                    foreach (OWLOntologyFact differentFact in differentFacts)
                    {
                        Dictionary<string, string> bindings = new Dictionary<string, string>();
            ...
            while (rowsEnum.MoveNext())
            {
                ...
                RDFPatternMember rightArgumentValue =
                    this.RightArgument is RDFVariable ? RDFQueryUtilities.ParseRDFPatternMember(currentRow[rightArgumentString].ToString())
                                                      : ((OWLOntologyFact)this.RightArgument).Value;
                        ...
                        && rightArgumentValue is RDFResource rightArgumentValueResource)
                {
                    ...
                    OWLOntologyFact leftFact = ontology.Data.SelectFact(leftArgumentValueResource.ToString());
                    if (leftFact == null)
                        leftFact = new OWLOntologyFact(leftArgumentValueResource);

                    //Search the right fact in the ontology
                    OWLOntologyFact rightFact = ontology.Data.SelectFact(rightArgumentValueResource.ToString());
                    if (rightFact == null)
                        rightFact = new OWLOntologyFact(rightArgumentValueResource);

                    //Protect atom's inferences with implicit taxonomy checks (only if taxonomy protection has been requested)
                    if (!options.EnforceTaxonomyProtection || !OWLOntologyHelper.CheckIsSameFactAs(ontology.Data, leftFact, rightFact))
                    {
                        //Create the inference as a taxonomy entry
                        OWLOntologyTaxonomyEntry sem_infA = new OWLOntologyTaxonomyEntry(leftFact, (OWLOntologyObjectProperty)this.Predicate, rightFact)
                                                                 .SetInference(RDFSemanticsEnums.OWLOntologyInferenceType.Reasoner);
                        OWLOntologyTaxonomyEntry sem_infB = new OWLOntologyTaxonomyEntry(rightFact, (OWLOntologyObjectProperty)this.Predicate, leftFact)
                                                                 .SetInference(RDFSemanticsEnums.OWLOntologyInferenceType.Reasoner);

                        //Add the inference to the report
                        if (!ontology.Data.Relations.DifferentFrom.ContainsEntry(sem_infA))
                            report.AddEvidence(new OWLOntologyReasonerEvidence(RDFSemanticsEnums.OWLOntologyReasonerEvidenceCategory.Data, this.ToString(), nameof(OWLOntologyData.Relations.DifferentFrom), sem_infA));
                        if (!ontology.Data.Relations.DifferentFrom.ContainsEntry(sem_infB))
                            report.AddEvidence(new OWLOntologyReasonerEvidence(RDFSemanticsEnums.OWLOntologyReasonerEvidenceCategory.Data, this.ToString(), nameof(OWLOntologyData.Relations.DifferentFrom), sem_infB));
                    }
                }
            }
