                    ...
                    foreach (RDFResource subClassOfHVRestriction in subClassesOfHVRestriction)
                    {
                        ...
                        foreach (RDFResource subClassOfHVRestrictionMember in subClassOfHVRestrictionMembers)
                        {
                            ...
                            OWLReasonerEvidence evidence = currentHVRestrictionValue is RDFResource
                                ? new OWLReasonerEvidence(OWLSemanticsEnums.OWLReasonerEvidenceCategory.Data, nameof(OWLHasValueEntailmentRule), new RDFTriple(subClassOfHVRestrictionMember, currentHVRestrictionOnProperty, (RDFResource)currentHVRestrictionValue))
                                : new OWLReasonerEvidence(OWLSemanticsEnums.OWLReasonerEvidenceCategory.Data, nameof(OWLHasValueEntailmentRule), new RDFTriple(subClassOfHVRestrictionMember, currentHVRestrictionOnProperty, (RDFLiteral)currentHVRestrictionValue));
